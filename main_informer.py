...
parser.add_argument('--logs', type=str, default='./logs', help='logs folder')
...
data_parser = {
    ...
    'DWV':{'data':'DWV.csv','T':'Returning.Visits','M':[10,10,10],'S':[1,1,1],'MS':[5,5,1]},
}
...
for ii in range(args.itr):
    ...
    #exp.train(setting)
    exp.train_LSTM(setting)
    
    
  
    print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
    exp.test_LSTM(setting)
